...
[Generator]
public class BindablePropertyAttributeSourceGenerator : BaseBindablePropertySourceGenerator
{
	...
	static BindablePropertySemanticValues SemanticTransform(GeneratorAttributeSyntaxContext context, CancellationToken cancellationToken)
	{
		...
		var @namespace = propertySymbol.ContainingNamespace.ToFQDisplayString();
		var className = propertySymbol.ContainingType.Name;
		var classAccessibility = propertySymbol.ContainingSymbol.DeclaredAccessibility.ToString().ToLower(CultureInfo.InvariantCulture);
	...
	static BindablePropertyModel CreateBindablePropertyModel(in AttributeData attributeData, in INamedTypeSymbol declaringType, in string propertyName, in ITypeSymbol returnType, in bool doesContainNewKeyword, in bool isReadOnly, in string? setterAccessibility, in bool hasInitializer, in string? propertyAccessibility)
	{
		...
		var coerceValueMethodName = GetNamedMethodGroupArgumentsAttributeValueByNameAsString(attributeData, nameof(BindablePropertyModel.CoerceValueMethodName));
		var defaultBindingMode = GetNamedTypeArgumentsAttributeValueForDefaultBindingMode(attributeData, nameof(BindablePropertyModel.DefaultBindingMode), "(Microsoft.Maui.Controls.BindingMode)0");
		var defaultValueCreatorMethodName = GetNamedMethodGroupArgumentsAttributeValueByNameAsString(attributeData, nameof(BindablePropertyModel.DefaultValueCreatorMethodName));
		var propertyChangedMethodName = GetNamedMethodGroupArgumentsAttributeValueByNameAsString(attributeData, nameof(BindablePropertyModel.PropertyChangedMethodName));
		var propertyChangingMethodName = GetNamedMethodGroupArgumentsAttributeValueByNameAsString(attributeData, nameof(BindablePropertyModel.PropertyChangingMethodName));
		var validateValueMethodName = GetNamedMethodGroupArgumentsAttributeValueByNameAsString(attributeData, nameof(BindablePropertyModel.ValidateValueMethodName));
